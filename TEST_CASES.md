...
Reference document for all 47 test cases in the model-signing conformance suite.

**Suite layout:**
- **27 `verify` tests** — use pre-committed bundles (signed offline, committed to this repo). Every client verifies the same bundles. Passing a verify test proves the client correctly implements the wire format and the verification logic for that scenario.
